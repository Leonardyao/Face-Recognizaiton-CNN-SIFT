...
filenames = []
labels = []
filedir = os.listdir(dirImage)
#print(filedir)
for file in filedir:
    labeldir = os.listdir(dirImage + file)
    for label in labeldir:
        imgfile = os.listdir(dirImage + file + "/"+ label)
        for img in imgfile:
            filenames.append(str(dirImage + file + "/" + label + "/" + img))
...
def get_sift(img):
    ...
    img = cv2.imread(img)
    img = cv2.resize(img, (245, 320), interpolation=cv2.INTER_CUBIC)
...
np.save('sift_image.npy', images)
#print(images)
'''
images = np.load('sift_image.npy')

temp = np.array([list(images), labels])
temp = temp.transpose()
shuffle(temp)
images = list(temp[:, 0])
...
emotion = ['1', '2', '3', '4', '5', '6', '7', '8'] #8 class
...
test_labels = np.asarray(labels[TRAINING_SIZE:], dtype=int)
...
train_image_width = 245
train_image_height = 320
...
conv2 = tf.layers.conv2d(pool1, filters=64, kernel_size=[3, 3], strides=[1, 1], padding='same',activation=tf.nn.relu,kernel_initializer=tf.truncated_normal_initializer(stddev=0.1),name='conv2')
...
conv3 = tf.layers.conv2d(pool1, filters=128, kernel_size=[3, 3], strides=[1, 1], padding='same',activation=tf.nn.relu,kernel_initializer=tf.truncated_normal_initializer(stddev=0.1),name='conv3')
...
conv4 = tf.layers.conv2d(pool1, filters=64, kernel_size=[3, 3], strides=[1, 1], padding='same',activation=tf.nn.relu,kernel_initializer=tf.truncated_normal_initializer(stddev=0.1),name='conv4')
...
#flattened = tf.reshape(layer3, [-1, 32 * 32 * 128])
...
with tf.Session() as sess:
    ...
    total_batch_test = int(len(test_labels) / batch_size)
    for epoch in range(epochs):
        ...
        for j in range(total_batch):
            ...
            _, c, train_acc = sess.run([optimiser, cross_entropy, accuracy], feed_dict={x:train_batch_images, y:train_batch_labels, keep_prob:DROPOUT})
            avg_cost += c
            batch_index += batch_size
        #train_acc = sess.run(accuracy, feed_dict={x:train_images, y:train_labels})
        for j in range(total_batch_test):
            test_batch_images = test_images[batch_index:(batch_index+batch_size), :]
            test_batch_labels = test_labels[batch_index:(batch_index+batch_size), :]
            test_acc = sess.run(accuracy, feed_dict={x: test_batch_images, y: test_batch_labels})
        print("Epoch:", (epoch + 1), "cost =", "{:.3f}".format(avg_cost), "train accuracy: {:.3f}".format(train_acc), "test accuracy: {:.3f}".format(test_acc))
        save_path = './model/ck-model.ckpt'
